...
class resnet50(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        tail = torch.mean(feature4, 3, keepdim=True)
        tail = torch.mean(tail, 2, keepdim=True)
        return feature3, feature4, tail  # 3 mid supervision
...
if __name__ == '__main__':
    model = build_contextpath('resnet18', pretrained=True)
    # img = torch.rand(1, 4, 480, 640)
    img = torch.rand(1, 4, 240, 320)
    f3, f4, tail = model.forward(img)
    print(f3.size())  # [1, 256, 30, 40]
    print(f4.size())  # [1, 512, 15, 20]
    print(tail.size())
