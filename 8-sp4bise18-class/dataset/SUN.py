...
class ToRGBD(object):
    def __call__(self, sample):
        image, depth, label, label_16, label_32 = sample['image'], sample['depth'], sample['label'], sample['label_16'], sample['label_32']
        rgbd = torch.cat((image, depth), 0)
        return rgbd, label, label_16, label_32


if __name__ == '__main__':
    dataset = Data(image_path='/temp_disk/xs/sun/train/image',
                   depth_path='/temp_disk/xs/sun/train/depth',
                   label_path='/temp_disk/xs/sun/train/label_npy',
                   transform=transforms.Compose([scaleNorm(),
                                                 ...
                                                 Normalize(),
                                                 ToRGBD()]))
    for i, (rgbd, label, label_16, label_32) in enumerate(dataset):
        print(rgbd.size())
        print(label.size())
        print(label_16.size())
        print(label_32.size())
