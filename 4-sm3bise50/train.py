...
def val(args, model, dataloader_val):
    ...
    with torch.no_grad():
        model.eval()
        precision_record = []
        for i, (data, label, _, _, _) in enumerate(dataloader_val):
            if torch.cuda.is_available() and args.use_gpu:
                data = data.cuda()
                label = label.cuda()
            # predict
            result, _, _, _ = model(data)
...
def train(args, model, optimizer, dataloader_train, dataloader_val):
    ...
    for epoch in range(args.epoch_start_i, args.num_epochs):
        ...
        loss_record = []
        for i, (data, label, label_8, label_16, label_32) in enumerate(dataloader_train):
            if torch.cuda.is_available() and args.use_gpu:
                data = data.cuda()
                label = label.cuda()
                label_8 = label_8.cuda()
                label_16 = label_16.cuda()
                label_32 = label_32.cuda()
            # mid supervision
            result, result_8, result_16, result_32 = model(data)
            results = (result, result_8, result_16, result_32)
            labels = (label, label_8, label_16, label_32)
            # multi scale
            losses = []
            ...
            loss = losses[0] + 0.2 * losses[1] + 0.5 * losses[2] + 0.8 * losses[3]  # deeper stage, deeper loss weight, extremely, output weight = 1
            # total loss
