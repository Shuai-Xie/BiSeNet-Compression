...
class BiSeNet(torch.nn.Module):
    def __init__(self, num_classes, context_path, phase_train=False):
        ...
        self.phase_train = phase_train
        self.cx0_mid_supervision_module = MidSupervisionModule(in_channels=512, num_classes=num_classes)
    ...
    def forward(self, input):
        ...
        cx0, cx1, cx2, tail = self.context_path(input)  # tail 1/32 fm global average
        ...
        if self.phase_train:  # should be here, because code below will change cx1,cx2
            result_8 = self.cx0_mid_supervision_module(cx0)  # 1/16 [1, 38, 60, 80]
        ...
        if self.phase_train:
            return result, result_8, result_16, result_32
        else:
            return result
