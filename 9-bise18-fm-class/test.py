...
parser.add_argument('--checkpoint_path', type=str, default='./checkpoints/epoch_105.pth',
                    help='The path to the pretrained weights of model')
parser.add_argument('--context_path', type=str, default="resnet18_s", help='The context path model you are using.')
parser.add_argument('--num_classes', type=int, default=14, help='num of object classes (with void)')  # todo
...
parser.add_argument('--csv_path', type=str, default='/home/disk1/xs/sun/seg14_class_dict.csv', help='Path to label info csv file')
...
args.crop_height, args.crop_width = 256, 320
...
def predict_RGBD(image, depth):  # nd convenient both for img and video
    ...
    # depth = torch.from_numpy(depth).float()  # sun
    depth = torch.from_numpy(depth * 10).float()  # kinect v1 danwei
...
def test_lab1():
    predict_img_dir(img_dir='./img/lab1/rgb/',
                    depth_dir='./img/lab1/depth/',
                    out_dir='./img/lab1/seg/')
